defmodule Day10 do
  ...
  def check_step_for_validity(
    ...
    not_previous_steps(row, column, previous_row, previous_column) == true and
      next_spot_climbable(matrix, row, column, current_row, current_column)
  end

  def get_only_valid_unseen(matrix, next_steps, current_pair, seen, scheduled_list) do
    Enum.filter(get_only_valid_unseen(matrix, next_steps, current_pair, seen), fn x ->
      Enum.member?(scheduled_list, x) == false
    end)
  end
  ...
  def conduct_path(start_row, start_column, matrix) do
    Enum.reduce_while(
      0..(tuple_size(matrix) * tuple_size(elem(matrix, 0))),
      %{:paths_counter => 0, :seen => [], :scheduled => [{start_row, start_column}]},
      fn _, acc ->
        if Map.get(acc, :scheduled) == [] do
          ...
          schedule_updated =
            Map.put(seen_updated, :scheduled, tl(Map.get(seen_updated, :scheduled)))

          if Utils.get_value_from_2d_tuple(matrix, elem(current_pair, 0), elem(current_pair, 1)) ==
               9 do
            ...
            {:cont,
             Map.put(
               schedule_updated,
               :paths_counter,
               Map.get(schedule_updated, :paths_counter) + 1
             )}
          else
            next_steps =
            ...
            filtered_steps =
              get_only_valid_unseen(
                matrix,
                next_steps,
                current_pair,
                Map.get(acc, :seen),
                Map.get(acc, :scheduled)
              )
            ...
            {:cont,
             Map.put(
               schedule_updated,
               :scheduled,
               filtered_steps ++ Map.get(schedule_updated, :scheduled)
             )}
          end
        end
      end
  ...
  def find_value_coords(matrix, target_value) do
    Enum.reduce(0..(tuple_size(matrix) - 1), [], fn x, acc ->
      acc ++
        Enum.reduce(0..(tuple_size(elem(matrix, x)) - 1), [], fn y, acc2 ->
