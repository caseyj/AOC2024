defmodule Day14 do
  ...
  def parse_position_and_slope(str) do
    captured =
      Regex.named_captures(
        ~r/p=(?<position_x>[-]?\d+),(?<position_y>[-]?\d+)\Wv=(?<slope_x>[-]?\d+),(?<slope_y>[-]?\d+)/,
        str
      )
  ...
  def get_all_points(str, turns, size_x, size_y) do
    Enum.reduce(str_to_point_slopes(str), [], fn pt_slope, acc ->
      new_spot =
        find_spot_after_n_turns(elem(pt_slope, 0), elem(pt_slope, 1), turns, size_x, size_y)
  ...
  def variance(var_list) do
    ...
    div(
      floor(
        Enum.reduce(var_list, 0, fn var, acc ->
  ...
  def variance_minima(str, size_x, size_y) do
    Enum.reduce(
      0..floor(:math.pow(size_x * size_y, 2)),
      {{100_000_000, 10_000_000}, 0, []},
      fn turns, acc ->
        points = get_all_points(str, turns, size_x, size_y)
        variances = get_variances(points)

        IO.puts(
          "Currently on #{turns}, with detected minima #{Utils.print_tuple(elem(acc, 0))}, detected variances #{Utils.print_tuple(variances)}"
        )

        if elem(variances, 0) < elem(elem(acc, 0), 0) or
             elem(variances, 1) < elem(elem(acc, 0), 1) do
