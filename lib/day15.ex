defmodule Day15 do
  ...
  def wall_collision(new_coords, wall_list) do
    ...
    all_vals = Enum.map(new_coords,
    fn x ->
      Enum.member?(wall_list, x)==true end)
    Enum.any?(all_vals)
  end
  ...
  def box_collision(object_coords, direction, box_list) do
    new_spot = Utils.direction_operator(
      elem(object_coords, 0),
      elem(object_coords,1),
      instruction_to_direction(direction), 1)
  ...
  def update_box_list(collisions,box_list ) do
    ...
    if length(collisions) == 1 do
      ...
      add_spots = Enum.reduce(tl(collisions), [], fn x, acc->
  ...
  def get_coordinate_sums(run_results)do
    Enum.reduce(elem(run_results,1), 0, fn coord, acc -> acc+(100*elem(coord,0))+elem(coord,1) end)
  end
