defmodule Day2 do
    ...
    if between_1_3(elem(list_ints, 0), elem(list_ints, 1)) == false do
      ...
      all_q =
        Enum.all?(index_list, fn x ->
          follows_rules(elem(list_ints, x - 1), elem(list_ints, x), previous_direction)
        end)
  ...
  def enumerate_options_for_list(list) do
    Enum.map(Enum.to_list(0..(Enum.count(list) - 1)), fn x ->
      create_options_for_index(list, x)
    end)
  end
  ...
  def atleast_1_safe(list) do
    ...
    if string_to_determination(lst) == :safe do
      ...
      Enum.any?(enumerate_options_for_list(Tuple.to_list(lst)), fn x ->
        string_to_determination(List.to_tuple(x)) == :safe
      end)
    end
  end
