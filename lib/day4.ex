...
def column_runner(map, row) do
  ...
  Enum.reduce(column, map, fn column, acc -> add_r_c_to_map(acc,elem(column,0), row, column) end)
end
...
def collect_valid_directions_from_start(board_map,start_row, start_column, letter_list, direction_list) do
  Enum.count(
    Enum.map(
      direction_list,
      fn x -> direction_collector(start_row, start_column, letter_list, x)
      end),
...
def find_all_xmas_from_x(board_map, letters) do
  Enum.sum(
    Enum.map(
      Map.get(board_map, "X"),
      fn x -> collect_valid_directions_from_start(
        board_map,
        elem(x,0),
        elem(x,1),
        letters,
...
def get_coords_for_x(start_row, start_column) do
  [
    [direction_operator(start_row,start_column,:northwest, 1),{start_row,start_column},direction_operator(start_row,start_column,:southeast,1)],
    [direction_operator(start_row,start_column,:northeast, 1),{start_row,start_column},direction_operator(start_row,start_column,:southwest,1)]
  ]
end
...
def has_letters_once(board_map, direction_coords) do
  ...
  forward = Enum.count(Map.get(board_map, "M"), fn x -> x==first end) + Enum.count(Map.get(board_map, "S"), fn x -> x==last end)
  backward = Enum.count(Map.get(board_map, "M"), fn x -> x==last end) + Enum.count(Map.get(board_map, "S"), fn x -> x==first end)
  forward == 2 or backward == 2
end

def check_x(board_map, start_row, start_column) do
  Enum.all?(get_coords_for_x(start_row, start_column), fn x -> has_letters_once(board_map, x) end)
end

def all_x_s(board_map) do
  Enum.count(Map.get(board_map, "A"), fn x -> check_x(board_map, elem(x, 0), elem(x,1))== true end)
end
