defmodule Day5 do
      ...
      if length(traversal) == 1 do
        ...
        if Enum.member?(collector, next_item) do
          ...
          if unvisited != [] do
            ...
            collected =
              get_corrected_traversal(reduced_unvisited, collector, rules_map, traverse_list)
      ...
      fn traversal, acc ->
        if check_update_list_is_valid(traversal, rules_map) do
          ...
          Map.put(
            acc,
            "incorrect",
            Map.get(acc, "incorrect") ++
              [get_corrected_traversal(traversal, [], rules_map, traversal)]
          )
        end
      end
    )
  ...
  def part1(filename) do
    ...
    sum_correct_traversals(
      parse_traversal(List.last(input_split)),
      parse_to_rule_map(List.first(input_split)),
      "correct"
    )
  end
