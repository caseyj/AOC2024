...
def guard_turn_w_direction(row, column, matrix, current_direction, size_x, size_y,guard_positions) do
  new_spot = Utils.direction_operator(row, column, current_direction, 1)
  if check_on_map(elem(new_spot, 0), elem(new_spot, 1),size_x, size_y) ==false do
    guard_positions
  else
    if is_block(elem(new_spot, 0), elem(new_spot, 1), matrix) do
      guard_turn_w_direction(row, column, matrix, rotate_guard(current_direction), size_x, size_y,guard_positions++[{row,column, rotate_guard(current_direction)}])
    else
      if Enum.member?(guard_positions, new_spot) == false do
        guard_turn_w_direction(elem(new_spot, 0), elem(new_spot, 1), matrix, current_direction,size_x, size_y, guard_positions++[{elem(new_spot, 0), elem(new_spot, 1), current_direction}])
      else
        guard_turn_w_direction(elem(new_spot, 0), elem(new_spot, 1), matrix, current_direction, size_x, size_y, guard_positions)
      end
    end
  end
end
...
def get_next_directions(row, column, direction, matrix) do
  ...
  case direction do
    x when  x==:north -> dynamic_static_numbers_to_list( range_to_max(row-1,0), column, :vertical)
    x when  x==:east-> dynamic_static_numbers_to_list( range_to_max(column+1, column_length), row, :horizontal)
    x when  x==:south -> dynamic_static_numbers_to_list( range_to_max(row+1,row_length-1), column, :vertical)
    x when  x==:west -> dynamic_static_numbers_to_list( range_to_max(column-1,0), row, :horizontal)
  end
end
...
def count_injectable_objects(matrix, found_path) do
  Enum.count(Enum.reduce(detect_block_causing_turns(found_path), [], fn x, acc ->
    acc++Enum.reduce(obstacle_inject_for_change(elem(x,0), elem(x,1), elem(x,2), matrix, found_path), [], fn y, acc2 ->
...
def obstacle_inject_for_change(row, column, direction,matrix, found_path) do
  next_directions = get_next_directions(
    ...
    matrix)
  next_spots = Enum.reduce(next_directions, [],fn x,acc ->
    acc++[{elem(x,0), elem(x, 1), previous_rotation(direction)}] end)
  repeats = Enum.reduce(next_spots, [], fn x, acc ->
...
def part1(filename) do
  ...
  Enum.count(get_path_from_matrix(matrix)) # off by 1 in actual answer, not sure on reason
end
