defmodule Day7 do
    ...
    if remaining_operations == [] do
      ...
      if result == target_value do
        ...
        if n_index < tuple_size(input_tuple) do
          options =
            tl(remaining_operations) ++
              build_options(result, elem(input_tuple, n_index), n_index, options_list)

          operation_runner(
            target_value,
            input_tuple,
            Enum.sort(options, &(l_one_dist(target_value, &1) <= l_one_dist(target_value, &2))),
            options_list
          )
        else
  ...
  def operation_runner(target_value, input_tuple, options_list) do
    operation_runner(
      target_value,
      input_tuple,
      build_options(elem(input_tuple, 0), elem(input_tuple, 1), 1, options_list),
      options_list
    )
  end
  ...
  def part1(filename) do
    ...
    count_correct(
      count_successful_operations(convert_input_str_to_list(content), [:sum, :mult], %{
        :correct => [],
        :incorrect => []
      })
    )
  ...
  def part2(filename) do
    ...
    initial_correct_track =
      count_successful_operations(parsed_input, [:sum, :mult], correctness_tracker)

    cats_only =
      count_successful_operations(parsed_input, [:sum, :mult, :cat], correctness_tracker)
