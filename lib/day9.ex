defmodule Day9 do
  ...
  def get_next_index_of_non_free_space(enum_w_index) do
    Enum.reduce_while(Enum.reverse(enum_w_index), 0, fn x, _ ->
      if elem(x, 0) != "." do
        {:halt, elem(x, 1)}
      else
        {:cont, 0}
          ...
          if elem(val, 0) == "." do
            acc =
              Map.put(
                ...
                swap(
                  Map.get(acc, :defrag),
                  elem(current_defrag_as_tuple, Map.get(acc, :seek)),
                  indx,
                  "."
                )
              )
            ...
            {:cont,
             Map.put(
               acc,
               :seek,
               get_next_index_of_non_free_space(Enum.with_index(Map.get(acc, :defrag)))
             )}
          else
            {:cont, acc}
          end
  ...
  def block_resort(blocks_list) do
    ...
    Enum.reduce_while(
      true,
      %{:defrag => indexed_blocks, :seek => 0, :indx_map => block_list_to_id_queue(blocks_list)},
      fn _, acc ->
  ...
  def part1(filename) do
    ...
    Day9.defrag_to_sum(
      Day9.swap_one_digit_at_a_time(Day9.build_accum(String.split(content, "", trim: true)))
    )
  end
