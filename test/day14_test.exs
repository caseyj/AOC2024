defmodule Day14Test do
  ...
  for {starting_position,slope,turns,size_x,size_y, expected} <- [
    ...
    test "Check find_spot_after_n_turns #{Utils.print_tuple(starting_position)}, slope #{Utils.print_tuple(slope)}, for turns #{turns}" do
      assert Day14.find_spot_after_n_turns(
        ...
        unquote(turns),
        unquote(size_x),
        unquote(size_y)) == unquote(expected)
    end
  end
  ...
  for {point, size_x, size_y, expected} <- [
    ...
    test "Check get_point_quadrant #{Utils.print_tuple(point)} sizes x #{size_x}, x #{size_y}" do
      assert Day14.get_point_quadrant(unquote(point), unquote(size_x), unquote(size_y)) == unquote(expected)
    end
  end

  test "Check run small example on count_quadrants" do
    input = "p=0,4 v=3,-3\np=6,3 v=-1,-3\np=10,3 v=-1,2\np=2,0 v=2,-1\np=0,0 v=1,3\np=3,0 v=-2,-2\np=7,6 v=-1,-3\np=3,0 v=-1,-2\np=9,3 v=2,3\np=7,3 v=-1,2\np=2,4 v=2,-3\np=9,5 v=-3,-3"
    assert Day14.count_quadrants(input, 100, 11,7) == %{:first=>1, :second=>4, :third=>1, :remove=>3,:fourth=> 3}
