defmodule Day4Test do
  ...
  test "Adding multiple elements to single Map accumulator" do
    assert Day4.column_runner(%{"X"=>[],"M"=>[],"A"=>[],"S"=>[]}, {["X", "M"],0}) == %{"X"=>[{0,0}], "M"=>[{0,1}],"A"=>[],"S"=>[]}
  end
  ...
  for {direction, distance, expected} <- [
    ...
    test "Check #{direction} direction generates correct results with magnitude #{distance}" do
      assert Day4.direction_operator(0,0,unquote(direction), unquote(distance)) == unquote(expected)
    end
  end
    ...
    test "Check correct number of valid XMAS detected" do
      assert Day4.collect_valid_directions_from_start(%{
        ...
        0,
        ["X","M","A","S"],
        [:north, :south, :east, :west, :northeast, :northwest, :southeast, :southwest]) == 1
    end

    test "Check board calculation is correct" do
      input = Day4.input_to_map(Day4.parse_input("MMMSXXMASM\nMSAMXMSMSA\nAMXSXMAAMM\nMSAMASMSMX\nXMASAMXAMM\nXXAMMXXAMA\nSMSMSASXSS\nSAXAMASAAA\nMAMMMXMMMM\nMXMXAXMASX"))
