defmodule Day5Test do
  ...
  for {update_instructions, expected} <- [
    ...
    test "Check that the instructions #{update_instructions} have a correct detection of #{expected}" do
      sample_rules = "47|53\n97|13\n97|61\n97|47\n75|29\n61|13\n75|53\n29|13\n97|29\n53|29\n61|53\n97|53\n61|29\n47|13\n75|47\n97|75\n47|61\n75|61\n47|29\n75|13\n53|13\n"
      assert Day5.check_update_list_is_valid(String.split(unquote(update_instructions), ",", trim: true), Day5.parse_to_rule_map(sample_rules)) == unquote(expected)
    end
  end
  ...
  for {input_list, expected} <- [
    ...
    test "Get corrected traversals for input #{input_list}" do
      ...
      assert Day5.get_corrected_traversal(
        ...
        [],
        rules_map,
        unquote(input_list)) == unquote(expected)
    end
  end
